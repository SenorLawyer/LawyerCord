...
export function ClientEventFields({ type, filter, choose, wait, onTypeChange, onChange }: Props) {
    const definition = isClientEventType(type) ? CLIENT_EVENTS[type] : undefined;
    return <>
        ...
        <span className="vc-ab-field-description">{definition?.description ?? "Choose a supported Discord event."}</span>
