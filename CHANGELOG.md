...
- Keep ProfileSets reads, saves, imports, and pending loads within their originating account, section, and list. Give each panel its own list so main and server loads cannot replace each other or export the wrong section. Reload on account changes, cancel dismissed imports, and reject outdated menu actions.
